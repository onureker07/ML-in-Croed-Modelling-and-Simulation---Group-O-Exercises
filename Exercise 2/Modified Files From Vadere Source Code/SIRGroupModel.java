...
public class SIRGroupModel extends AbstractGroupModel<SIRGroup> {
	...
	long last_updated = -1L;
	@Override
	public void update(final double simTimeInSec) {
			if(last_updated == Math.round(simTimeInSec)) return;
			last_updated = Math.round(simTimeInSec);
			// check the positions of all pedestrians and switch groups to INFECTED (or REMOVED).
			DynamicElementContainer<Pedestrian> c = topography.getPedestrianDynamicElements();
			for(Pedestrian p: c.getElements()){
				SIRGroup group = getGroup(p);
				//Recovered check
				if(group.getID() == SIRType.ID_INFECTED.ordinal() && this.random.nextDouble() < attributesSIRG.getRecoverRate()){
					elementRemoved(p);
					assignToGroup(p, SIRType.ID_RECOVERED.ordinal());
				}

				//Infection Check
				List<Pedestrian> neighbors = c.getCellsElements().getObjects(p.getPosition(),
						this.attributesSIRG.getInfectionMaxDistance());
				for(Pedestrian neigh: neighbors){
					if(neigh == p) continue;
					if(this.random.nextDouble() < attributesSIRG.getInfectionRate() && getGroup(neigh).getID()
					== SIRType.ID_INFECTED.ordinal()){
						if (group.getID() == SIRType.ID_SUSCEPTIBLE.ordinal()) {
							elementRemoved(p);
